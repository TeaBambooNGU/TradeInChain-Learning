...
/** 
 * Uniswapv2 swap逻辑拆解 tokenA 换 tokenB
 * 1. tokenA的owner 授权给 Uniswapv2 调用所需交换数量的token的权限
 * 2. 如果参与交换的两种token没有WETH，则需要WETH作为中间桥梁进行token交换
 * 3. tokenA的合约 交易 所需交换数量的token给 UniswapV2Pair合约地址C
 * 
 * 4. UniswapV2Pair合约地址C收到tokenA后，再执行swap逻辑，
 * 4. 把拥有WETH的UniswapV2Pair合约地址C的WETH 通过WETH合约 交易 所需数量的WETH 到 UniswapV2Pair合约地址D
 * 5. UniswapV2Pair合约地址C 记录当下UniswapV2Pair合约地址C中的 tokenA和WETH的余额 并发送Sync 和 Swap 事件
 * 
 * 6. UniswapV2Pair合约地址D 执行swap逻辑 
 * 6. 把UniswapV2Pair合约地址D中的tokenB 通过tokenB的合约 交易到 tokenA的owner地址 实现了对于该owner的token交换
 * 7. UniswapV2Pair合约地址D 记录当下UniswapV2Pair合约地址D中的 tokenB和WETH的余额 并发送Sync 和 Swap 事件
 * 
 */
contract ContractTest is Test {
  ...
  string mainnet = "https://eth-mainnet.g.alchemy.com/v2/yxxxxx";
  ...
  function swap(
    ...
    if (_tokenIn == address(WETH) || _tokenOut == address(WETH) ) {
      ...
      // 如果参与交换的两种token没有WETH，则需要WETH作为中间桥梁进行token交换
      path = new address[](3);
      path[0] = _tokenIn;
      path[1] = address(WETH) ;
      path[2] = _tokenOut;
